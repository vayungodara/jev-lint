...
Counts: 5 high, 12 medium (11 fixed, M8 withdrawn), 9 low, 13 recorded but not fixed, 4 proposed fixes rejected. Two oracle passes (bottom) added H6, M10–M12 and L8, downgraded H3 to L9, and overturned two of my fixes.
...
My first fence fix toggled on either marker, so `~~~` inside a ```` ``` ```` block ended the block and the rest of the code became claim text. Main did not have this bug. Fix: `visible_lines` records the opening run (character and length) and closes only on a run of the same character at least as long, which also handles ```` ```` ```` fences that contain ```` ``` ````. After the second oracle pass it also follows CommonMark on two more points: a line indented four or more spaces is never a fence, and a backtick opener whose info string contains a backtick (```` ```x``` ````) is an inline span, not a fence. Inline spans are found by a small run scanner (`mask_code_spans`): whole backtick runs pair only with the next run of exactly the same length, an escaped opener does not count, and spans are masked with spaces instead of removed, so `TO`x`DO` no longer collapses into a marker and link positions stay aligned with the original line. Multi-line spans are not handled (Recorded 12). Tests: `test_line_numbers_skip_frontmatter_and_fences`, `test_fence_edge_cases_follow_commonmark`, `test_markers_inside_inline_code_are_ignored`, `test_code_span_runs_must_match_exactly`, `test_wikilink_resolution` line 8.
...
The overwrite guard ran outside the `try` that maps `OSError` to exit 2, and read the whole existing file to check 500 bytes. Fix: `main()` wraps the whole CLI body (argument parsing, `lint()`, a final stdout flush) in one error boundary; the guard reads only 500 characters; report and cache go through `atomic_write()` (`tempfile.mkstemp` in the target directory, so a stale or symlinked `.tmp` is never truncated; on failure the temp file is removed and the destination untouched). A closed stdout (`jev-lint … | head`) previously ended with "Exception ignored … BrokenPipeError" and exit 120; it now exits 2 quietly. Tests: `test_cli_exit_codes_and_report` (`-o <dir>` exits 2 with "Is a directory"), `test_atomic_write_failure_leaves_destination_and_no_temp`, `test_closed_stdout_exits_cleanly_without_traceback` (subprocess with a closed pipe; exit 120 on main).
...
8. **Fail-fast has a gap (oracle).** `Executor.map` yields in submission order: if question 0 is slow (up to the 60 s timeout, longer with retries) while question 1 fails, the other 7 workers keep working through the queue until question 0 returns, so in the worst case most of the vault is still billed. Ctrl-C during the submission phase also waits for queued work. Failures that would trigger this (401, network down) fail every request quickly, so the realistic cost is small; the fix (`as_completed` with explicit cancellation) is more code than it saves for a patch release.
...
12. **Inline code spans are matched per line**, so a span that continues on the next line is not masked. Fences cover the multi-line case; the per-line limit is documented here rather than guessed at across paragraphs.
13. **Cache entries written by 0.1.0 are not re-validated on load**; a malformed one (only possible if the API returned garbage) would fail with "unexpected answer" until the cache file is deleted. 0.1.1 never writes such entries.
...
A second, narrower pass reviewed only the code written after the first pass. It found the indented-closer and backtick-info-string fence cases, the partial-run and escaped-opener weaknesses of the code-span regex, and the broken-pipe exit path (all fixed, see M11 and L8); it asked for the key test to use the characters urllib actually chokes on (done), for `OverflowError` in the answer parser (done), for an `atomic_write` failure test (done), and for the fail-fast rationale in Recorded 8 to stop claiming a per-minute bound (corrected). It judged the three rejections defensible for a patch release and confirmed that the answer validation cannot reject a documented TypeSafe HTTP answer (string-keyed `probabilities`; noul without `confidence`).
